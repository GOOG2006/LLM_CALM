"""Stage 4 of the LVH pipeline: LoRA + residual verdict head
   logit = Linear(h_last) + MLP(h_mid^step-boundary), where the MID layer is the one that
   Stage 2/3 selected (read from selected_layer.txt, or the LVH_LAYER env var, default -9).
Backbone = DSR1-7B (base, no GenPRM). Trained on GenPRM-Data step Yes/No labels.
Eval on the same 120-case slices as the generative PRM (pb_*_7b_out). bf16 + gradient checkpointing.
Usage: python stage4_train_lvh.py [n_conv] [nsteps]
"""
...
# mid-layer tap chosen by the self-configuration step (Stage 2/3)
L9 = int(open('selected_layer.txt').read().strip()) if os.path.exists('selected_layer.txt') else int(os.environ.get('LVH_LAYER', -9))
print(f"[stage4] mid-layer tap L9 = {L9}", flush=True)
